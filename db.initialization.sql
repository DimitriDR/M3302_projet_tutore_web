...
CREATE OR REPLACE DEFINER = root@localhost VIEW projet_tutore_web.`backoffice.orders` AS
SELECT `projet_tutore_web`.`orders`.`id_order`     AS `id_order`,
       `projet_tutore_web`.`users`.`email_address` AS `email_address`,
...
CREATE OR REPLACE DEFINER = root@localhost VIEW projet_tutore_web.`view.users` AS
SELECT `projet_tutore_web`.`users`.`id_user`                 AS `id_user`,
       `projet_tutore_web`.`users`.`last_name`               AS `last_name`,
       `projet_tutore_web`.`users`.`first_name`              AS `first_name`,
       `projet_tutore_web`.`users`.`password`                AS `password`,
       `projet_tutore_web`.`users`.`street_name`             AS `street_name`,
       `projet_tutore_web`.`users`.`zip_code`                AS `zip_code`,
       `projet_tutore_web`.`users`.`district`                AS `district`,
       `projet_tutore_web`.`users`.`city`                    AS `city`,
       `projet_tutore_web`.`users`.`mobile_number`           AS `mobile_number`,
       `projet_tutore_web`.`users`.`email_address`           AS `email_address`,
       `projet_tutore_web`.`users.payment`.`id_payment`      AS `id_payment`,
       `projet_tutore_web`.`users.payment`.`name`            AS `name`,
       `projet_tutore_web`.`users.payment`.`number`          AS `number`,
       `projet_tutore_web`.`users.payment`.`cvv`             AS `cvv`,
       `projet_tutore_web`.`users.payment`.`expiration_date` AS `expiration_date`
FROM (`projet_tutore_web`.`users`
         JOIN `projet_tutore_web`.`users.payment`
              ON (`projet_tutore_web`.`users`.`id_user` = `projet_tutore_web`.`users.payment`.`id_user`));