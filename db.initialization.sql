...
(
    ...
    COMMENT 'Table contenant tous les paniers composées par le producteur';
...
CREATE OR REPLACE TABLE `products.inventory`
(
...
CREATE OR REPLACE TABLE products_basket
(
    id_product int NOT NULL,
    id_basket  int NOT NULL,
    PRIMARY KEY (id_product, id_basket),
    CONSTRAINT products_basket_baskets_id_basket_fk
        FOREIGN KEY (id_basket) REFERENCES baskets (id_basket),
    CONSTRAINT products_basket_products_id_product_fk
        FOREIGN KEY (id_product) REFERENCES products (id_product)
);
...
CREATE OR REPLACE TABLE products_orders
(
    id_product int NOT NULL,
    id_order   int NOT NULL,
...
CREATE OR REPLACE TABLE `users.payment`
(
...
CREATE OR REPLACE DEFINER = root@localhost VIEW `backoffice.orders` AS
SELECT `projet_tutore_web`.`orders`.`id_order`   AS `id_order`,
       ...
       `projet_tutore_web`.`orders`.`amount`     AS `amount`
...
CREATE OR REPLACE DEFINER = root@localhost VIEW `backoffice.products` AS
